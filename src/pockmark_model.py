...
Author: Callum Hood-Cree (https://github.com/callumhc)

The model tests whether residual basal overpressure consistent with glaciogenic
emplacement ~17 ka (Siegel et al., 2014a, 2014b; Person et al., 2003) under a
mud cap can sustain upward exit gradients exceeding the Terzaghi heave
criterion (Freeze & Cherry, 1979) in the mud zone while the adjacent sand
flanks have already discharged. This is used as a permissive screen for
late-Holocene mud-only pockmark formation at the New England Mud Patch
(Goff, 2019; Goff et al., 2019).
...
# Project layout: this file lives in <root>/src/, so the project root is one
# level up. The MODFLOW-6 binary lives in <root>/bin/.
PROJECT_ROOT = Path(__file__).resolve().parent.parent
MF6_EXE = PROJECT_ROOT / "bin" / "mf6.exe"
